...
if ($w == '' && run_replace('register_member_chk_captcha', !chk_captcha(), $w)) {
...
if ($w == 'u') {
    ...
    $mb_zip1 = $member['mb_zip1'];
    $mb_zip2 = $member['mb_zip2'];
    ...
    $mb_sms = $member['mb_sms'];
    $mb_open = $member['mb_open'];
    $mb_marketing_agree = $member['mb_marketing_agree'];
    $mb_thirdparty_agree = $member['mb_thirdparty_agree'];
    $mb_2 = $member['mb_2'];
    $mb_4 = $member['mb_4'];
    $mb_5 = $member['mb_5'];
    $mb_6 = $member['mb_6'];
...
if ($w == '' || $w == 'u') {
    ...
    if ($w=='') {
        ...
        // 닉네임수정일이 지나지 않았다면
        if (empty($_POST['willow_profile_edit']) && $member['mb_nick_date'] > date("Y-m-d", G5_SERVER_TIME - ($config['cf_nick_modify'] * 86400)))
...
if ($w == '') {
    ...
    if ($old_email != $mb_email && $config['cf_use_email_certify']) {
        ...
        alert('회원 정보가 수정 되었습니다.', G5_URL.'/willow/menu.php');
    }
}
