...
if ($w == "") {
    ...
    */
    $willow_member_confirmed = get_session('ss_willow_member_confirm_'.$member['mb_id']);
    $willow_member_confirmed = $willow_member_confirmed && $willow_member_confirmed >= (G5_SERVER_TIME - 600);
    
    if(!$willow_member_confirmed && $_POST['mb_id'] && ! (isset($_POST['mb_password']) && $_POST['mb_password'])){
        if( ! $is_social_login_modify ){
            alert('비밀번호를 입력해 주세요.');
        }
    }

    if (!$willow_member_confirmed && isset($_POST['mb_password'])) {
